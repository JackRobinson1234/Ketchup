...
class RestaurantViewModel: ObservableObject {
    ...
    @Published var similarRestaurants: [Restaurant] = []
        @Published var isLoadingSimilarRestaurants = false
    ...
    func fetchSimilarRestaurants() async {
            guard let restaurant = self.restaurant,
                  let peopleAlsoSearch = restaurant.peopleAlsoSearch,
                  !peopleAlsoSearch.isEmpty else { return }
            ...
            do {
                ...
                for item in peopleAlsoSearch {
                    if let title = item.title,
                       let fetchedRestaurant = try await RestaurantService.shared.fetchRestaurant(byName: title, nearGeoHash: restaurant.geoHash ?? "") {
