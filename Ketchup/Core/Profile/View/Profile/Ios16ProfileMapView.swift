...
struct Ios16ProfileMapView: View {
    @StateObject var viewModel = Ios16ProfileMapViewModel()
    ...
    @State var selectedCluster: Ios16ProfilePostClusterAnnotation?
        ...
        DispatchQueue.main.async {
            ...
            for insertion in difference.insertions {
                ...
                case .cluster(let newItem):
                    self.clusters.append(Ios16ProfilePostClusterAnnotation(
                        id: newItem.id,
...
class Ios16ProfilePostClusterAnnotation: NSObject, Identifiable, MKAnnotation, CoordinateIdentifiable {
    var id = UUID()
    var coordinate: CLLocationCoordinate2D
    var count: Int
...
struct ProfileKitMapView: UIViewRepresentable {
    @ObservedObject var viewModel: Ios16ProfileMapViewModel
    @Binding var selectedWrittenPost: Post?
    @Binding var selectedCluster: Ios16ProfilePostClusterAnnotation?
    var mapSize: CGSize
    ...
    func makeUIView(context: Context) -> MKMapView {
        ...
        mapView.register(Ios16ProfilePostClusterAnnotationView.self, forAnnotationViewWithReuseIdentifier: Ios16ProfilePostClusterAnnotationView.identifier)
    ...
    class Coordinator: NSObject, MKMapViewDelegate {
        ...
        var viewModel: Ios16ProfileMapViewModel
        var debouncer = Debouncer(delay: 0.5)

        init(_ parent: ProfileKitMapView, viewModel: Ios16ProfileMapViewModel) {
            self.parent = parent
        ...
        func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
            ...
            } else if let clusterAnnotation = annotation as? Ios16ProfilePostClusterAnnotation {
                let identifier = Ios16ProfilePostClusterAnnotationView.identifier
                var annotationView = mapView.dequeueReusableAnnotationView(withIdentifier: identifier) as? Ios16ProfilePostClusterAnnotationView
                if annotationView == nil {
                    annotationView = Ios16ProfilePostClusterAnnotationView(annotation: annotation, reuseIdentifier: identifier)
                } else {
                    annotationView?.annotation = annotation
                }
                return annotationView
        ...
        func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
            ...
            } else if let clusterAnnotation = view.annotation as? Ios16ProfilePostClusterAnnotation {
...
class ProfilePostAnnotationView: MKAnnotationView {
    ...
    private var hostingController: UIHostingController<Ios16ProfileSinglePostAnnotationView>?
    ...
    private func configure(with annotation: ProfilePostAnnotation) {
        ...
        let clusterCell = Ios16ProfileSinglePostAnnotationView(post: annotation.post)
...
class Ios16ProfilePostClusterAnnotationView: MKAnnotationView {
    static let identifier = "Ios16ProfilePostClusterAnnotationView"

    private var hostingController: UIHostingController<Ios16ProfilePostClusterCell>?

    override var annotation: MKAnnotation? {
        willSet {
            guard let clusterAnnotation = newValue as? Ios16ProfilePostClusterAnnotation else { return }
            configure(with: clusterAnnotation)
        }
    }

    private func configure(with cluster: Ios16ProfilePostClusterAnnotation) {
        canShowCallout = false
        self.subviews.forEach { $0.removeFromSuperview() }

        let clusterCell = Ios16ProfilePostClusterCell(cluster: cluster)
        let hostingController = UIHostingController(rootView: clusterCell)
        hostingController.view.translatesAutoresizingMaskIntoConstraints = false
        self.addSubview(hostingController.view)
        self.hostingController = hostingController
...
struct Ios16ProfileSinglePostAnnotationView: View {
    let post: Post
    ...
    var body: some View {
        if post.mediaType == .written || post.mediaUrls.isEmpty {
            ...
            Ios16PostAnnotationView(post: post)
        }
    }
}

struct Ios16PostAnnotationView: View {
    let post: Post
...
struct Ios16ProfilePostClusterCell: View {
    var cluster: Ios16ProfilePostClusterAnnotation
