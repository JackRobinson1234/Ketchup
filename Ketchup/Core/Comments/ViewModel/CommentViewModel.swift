...
import SwiftUI
@MainActor
class CommentViewModel<T: Commentable>: ObservableObject {
    ...
    var commentCountText: String {
        return "\(commentable.commentCount) comments"
    }

    init(commentable: Binding<T>) {
        self._commentable = commentable
    }

    func initiateReply(to comment: Comment, replyToUser: String) {
        // If replying to a reply, reset the id to the original comment
    ...
    // Fetch comments for the current commentable (Post or Poll)
    func fetchComments() async throws {
        Task {
            let fetchedComments = try await CommentService.shared.fetchComments(for: commentable)
            let likedComments = try await checkIfUserLikedComments(comments: fetchedComments)
            organizeComments(likedComments)
        }
    }
    ...
    private func organizeComments(_ comments: [Comment]) {
        ...
        for comment in comments {
            if let replyTo = comment.replyTo {
                repliesDictionary[replyTo.commentId, default: []].append(comment)
            } else {
        ...
        for (index, topLevelComment) in topLevelComments.enumerated() {
            if let replies = repliesDictionary[topLevelComment.comment.id] {
                topLevelComments[index].replies = replies.sorted(by: {
        ...
        DispatchQueue.main.async {
            ...
            self.organizedComments = topLevelComments.sorted(by: {
               
                return $0.comment.timestamp.dateValue() < $1.comment.timestamp.dateValue()
            })
            self.showEmptyView = self.organizedComments.isEmpty
    ...
    func deleteComment(comment: Comment) async throws {
        try await CommentService.shared.deleteComment(comment: comment, from: commentable)

        DispatchQueue.main.async {
            if let index = self.organizedComments.firstIndex(where: { $0.comment.id == comment.id }) {
                // Main comment: delete it along with all its replies
                let commentsToRemoveCount = 1 + self.organizedComments[index].replies.count
                self.organizedComments.remove(at: index)
                self.$commentable.wrappedValue.commentCount -= commentsToRemoveCount
            } else {
                // Reply: find and remove it from the appropriate replies list
    ...
    func like(_ comment: Comment) async {
        do {
            try await CommentService.shared.likeComment(comment, in: commentable)
            await MainActor.run {
    ...
    func unlike(_ comment: Comment) async {
        do {
            try await CommentService.shared.unlikeComment(comment, in: commentable)
            await MainActor.run {
    ...
    func checkIfUserLikedComments(comments: [Comment]) async throws -> [Comment] {
        var updatedComments = [Comment]()
        for var comment in comments {
            do {
                let didLike = try await CommentService.shared.checkIfUserLikedComment(comment, in: commentable)
